...
import { nothing as o, html as s } from "lit";
...
class C extends d {
  ...
  render() {
    ...
    return u({ disabled: this.disabled, children: s`<td class=${i({
    ...
    })} colSpan=${this.colSpan ?? o} @blur=${this.onContainerBlur} @focus=${this.onContainerFocus} .role=${this.interactionMode === "interactive" ? "gridcell" : "cell"} rowSpan=${this.rowSpan ?? o} .tabIndex=${l ? -1 : 0} ${r(this.containerEl)}>${(this.selectionCell || this.readCellContentsToAT) && s`<span .ariaLive=${this.focused ? "polite" : "off"} class=${i(e.assistiveText)}>${this.selectionCell && this.selectionText || ""}${this.readCellContentsToAT && !this.selectionCell && this.contentsText || ""}</span>` || ""}<slot @slotchange=${this.updateScreenReaderContentsText}></slot></td>` });
  }
}
