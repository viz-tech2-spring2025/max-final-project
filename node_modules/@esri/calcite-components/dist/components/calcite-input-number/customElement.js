import { C as E, c as S } from "../../chunks/runtime.js";
import { live as H } from "lit-html/directives/live.js";
import { keyed as B } from "lit-html/directives/keyed.js";
import { html as d, nothing as h } from "lit";
import { createRef as k, ref as y } from "lit-html/directives/ref.js";
import { LitElement as F, createEvent as g, stringOrBoolean as I, safeClassMap as a } from "@arcgis/lumina";
import { useWatchAttributes as O } from "@arcgis/components-controllers";
import { v as w, i as z, g as D } from "../../chunks/dom.js";
import { c as C, i as N, d as T, s as P, H as j } from "../../chunks/form.js";
import { u as L, I as M } from "../../chunks/interactive.js";
import { n as W } from "../../chunks/key.js";
import { c as U, d as A, g as K } from "../../chunks/label.js";
import { c as _, g as v } from "../../chunks/component.js";
import { i as p, n as s, B as R, p as q, s as G, c as Y } from "../../chunks/locale.js";
import { V as Z } from "../../chunks/Validation.js";
import { s as J } from "../../chunks/input.js";
...
class it extends F {
  ...
  static {
    this.properties = { displayedValue: 16, slottedActionElDisabledInternally: 16, alignment: 3, autocomplete: 0, clearable: 7, disabled: 7, editingEnabled: 7, form: 3, groupSeparator: 7, icon: [3, { converter: I }], iconFlipRtl: 7, integer: 5, label: 1, loading: 7, localeFormat: 5, max: 11, maxLength: 11, messageOverrides: 0, min: 11, minLength: 11, name: 3, numberButtonType: 3, numberingSystem: 3, placeholder: 1, prefixText: 1, readOnly: 7, required: 7, scale: 3, status: 3, step: 3, suffixText: 1, validationIcon: [3, { converter: I }], validationMessage: 1, validity: 0, value: 1 };
  }
  static {
    this.styles = et;
  }
  ...
  async setFocus() {
    await _(this), this.childNumberEl?.focus();
  }
  connectedCallback() {
    super.connectedCallback(), this.inlineEditableEl = this.el.closest("calcite-inline-editable"), this.inlineEditableEl && (this.editingEnabled = this.inlineEditableEl.editingEnabled || !1), U(this), C(this), this.el.addEventListener(N, this.onHiddenFormInputInput);
  }
  async load() {
  ...
  disconnectedCallback() {
    super.disconnectedCallback(), A(this), T(this), this.el.removeEventListener(N, this.onHiddenFormInputInput);
  }
  // #endregion
  ...
  keyDownHandler(t) {
    this.readOnly || this.disabled || t.defaultPrevented || (this.isClearable && t.key === "Escape" && (this.clearInputValue(t), t.preventDefault()), t.key === "Enter" && P(this) && t.preventDefault());
  }
  onLabelClick() {
    this.setFocus();
  }
  ...
  setNumberValue({ committing: t = !1, nativeEvent: e, origin: i, previousValue: m, value: n }) {
    ...
    const l = this.previousValue?.length > n.length || this.value?.length > n.length, u = this.integer ? n.replace(/[e.]/g, "") : n, b = u.charAt(u.length - 1) === ".", f = b && l ? u : G(u), o = n && !f ? p(this.previousValue) ? this.previousValue : "" : f;
    let c = s.localize(o);
    i !== "connected" && !b && (c = Y(c, o, s)), this.displayedValue = b && l ? `${c}${s.decimal}` : c, this.setPreviousNumberValue(m ?? this.value), this.previousValueOrigin = i, this.userChangedValue = i === "user" && this.value !== o, this.value = ["-", "."].includes(o) ? "" : o, i === "direct" && (this.setInputNumberValue(c), this.setPreviousEmittedNumberValue(c)), e && (this.calciteInputNumberInput.emit().defaultPrevented ? (this.value = this.previousValue, this.displayedValue = s.localize(this.previousValue)) : t && this.emitChangeIfUserModified());
  }
  ...
  render() {
    const t = D(this.el), e = d`<div class=${a(r.loader)}><calcite-progress .label=${this.messages.loading} type=indeterminate></calcite-progress></div>`, i = d`<button .ariaLabel=${this.messages.clear} class=${a(r.clearButton)} .disabled=${this.disabled || this.readOnly} @click=${this.clearInputValue} tabindex=-1 type=button><calcite-icon icon=x .scale=${v(this.scale)}></calcite-icon></button>`, m = d`<calcite-icon class=${a(r.inputIcon)} .flipRtl=${this.iconFlipRtl} .icon=${this.requestedIcon} .scale=${v(this.scale)}></calcite-icon>`, n = this.numberButtonType === "horizontal", l = d`<button aria-hidden=true class=${a({
      ...
      [r.numberButtonItem]: !0,
      [r.buttonItemHorizontal]: n
    })} data-adjustment=down .disabled=${this.disabled || this.readOnly} @pointerdown=${this.nudgeButtonPointerDownHandler} @pointerout=${this.nudgeButtonPointerOutHandler} @pointerup=${this.nudgeButtonPointerUpHandler} tabindex=-1 type=button><calcite-icon icon=chevron-down .scale=${v(this.scale)}></calcite-icon></button>`, b = d`<div class=${a(r.numberButtonWrapper)}>${l}${u}</div>`, f = d`<div class=${a(r.prefix)}>${this.prefixText}</div>`, o = d`<div class=${a(r.suffix)}>${this.suffixText}</div>`, c = B("localized-input", d`<input aria-errormessage=${V.validationMessage} .ariaInvalid=${this.status === "invalid"} .ariaLabel=${K(this)} autocomplete=${this.autocomplete ?? h} .autofocus=${this.el.autofocus} value=${this.defaultValue ?? h} .disabled=${this.disabled} enterkeyhint=${this.el.enterKeyHint ?? h} inputmode=${this.el.inputMode || "decimal"} maxlength=${this.maxLength ?? h} minlength=${this.minLength ?? h} name=${h} @blur=${this.inputNumberBlurHandler} @focus=${this.inputNumberFocusHandler} @input=${this.inputNumberInputHandler} @keydown=${this.inputNumberKeyDownHandler} @keyup=${this.inputNumberKeyUpHandler} placeholder=${(this.placeholder || "") ?? h} .readOnly=${this.readOnly} type=text .value=${H(this.displayedValue ?? "")} ${y(this.setChildNumberElRef)}>`);
    return M({ disabled: this.disabled, children: d`<div class=${a({ [r.inputWrapper]: !0, [E.rtl]: t === "rtl" })} ${y(this.inputWrapperEl)}>${this.numberButtonType === "horizontal" && !this.readOnly ? u : null}${this.prefixText ? f : null}<div class=${a(r.wrapper)}>${c}${this.isClearable ? i : null}${this.requestedIcon ? m : null}${this.loading ? e : null}</div><div class=${a(r.actionWrapper)} ${y(this.actionWrapperEl)}><slot name=${tt.action}></slot></div>${this.numberButtonType === "vertical" && !this.readOnly ? b : null}${this.suffixText ? o : null}${this.numberButtonType === "horizontal" && !this.readOnly ? l : null}${j({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? Z({ icon: this.validationIcon, id: V.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
S("calcite-input-number", it);
export {
  it as InputNumber
};
